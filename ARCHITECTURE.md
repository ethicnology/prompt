...
`review` owns a read-only review of an existing session diff. For OpenCode 1.18.25, the PoC resolves the latest non-empty user-message diff within the current session before freezing an immutable snapshot. There is no arbitrary file-count or patch-size bound: selected model context/input limits are reported and the virtualized reusable `DiffViewer` prevents rendering size from becoming a product limit. A review starts with two independent child-session passes, correctness and security, and the user may add the tests/regressions pass. Every active pass requires a distinct selected model; defaults maximize provider diversity among the connected models reported by the user's OpenCode configuration before falling back to another model from an already selected provider. Children are created with `parentID`, a deny-all permission ruleset, and tool-free prompts. The complete diff is embedded as untrusted repository data and the request uses OpenCode JSON Schema structured output.

The review repository reconciles bounded REST polling of session status and messages. A completed assistant structured result is required before success; idle status alone is insufficient. Each pass has a 30-minute default timeout and concurrent pass execution has a 35-minute global bound after the snapshot and child sessions are prepared. A timed-out pass immediately requests the official abort and cleanup is confirmed before reporting success; cleanup uncertainty remains a typed partial failure. One repository serializes its runs, preserves successful opinions, and never retries a provider generation automatically. Typed state preserves partial failures, timeout, cancellation, metrics, provider/model provenance, hypotheses, and disagreement groups while retaining each original opinion. Final runs are stored in encrypted native Drift v6 history (Web is in-memory), and history is reopened read-only by profile and session. The setup computes an approximate, theoretical input-token and USD estimate from the complete serialized prompt, including schema, instructions, and diff, and identifies unknown prices and model-limit violations. Review never writes code, tests, workspaces, or Git, and never auto-confirms, fixes, commits, or merges. Debate/adjudication beyond disagreement grouping is deferred. The facade and presentation-neutral controller expose this flow for an adaptive Android UI without importing transport into the view model.
