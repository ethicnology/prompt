...
- Android and Linux use encrypted SQLite with a random database key held only in platform secure storage.
- Web does not provide equivalent native SQLite-at-rest encryption. Conversation-content persistence is disabled by default there; metadata may persist.
